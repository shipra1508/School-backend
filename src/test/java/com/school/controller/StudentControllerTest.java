...
@WithMockUser(username = "Shipra", roles = {"STUDENT", "TEACHER"})
class StudentControllerTest {
    ...
    void fetchAllStudents() throws Exception {
        ...
        mockMvc.perform(get("/students/all")
            ...
            .andExpect(jsonPath("$[0].name").value("Shipra"))
            .andExpect(jsonPath("$[1].name").value("Shree"));
    }
    ...
    void getStudentById() throws Exception {
        ...
        mockMvc.perform(get("/students/view/1")
                .with(csrf()))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.name").value("Shipra"))
            .andExpect(jsonPath("$.studentClass").value("Grade 10"));
    }
